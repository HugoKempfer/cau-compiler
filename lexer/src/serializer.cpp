...
void print_tokens(std::vector<Token> const &tokens) {
  if (!tokens.empty() && tokens.back().type == NONE) {
    ...
    std::cerr << "[ "
              << error.substr(0, error_end_index ? error_end_index
                                                 : error.length())
              << " ]" << std::endl;
  ...
  for (auto const &token : tokens) {
    if (token.type == WHITESPACE || token.type == NONE)
      continue;
    std::cout << "<" << TokenNames.at(token.type) << ", " << token.value << ">"
              << std::endl;
  }
}
...
void write_error_report(std::ofstream &file, const std::vector<Token> &tokens) {
  ...
  auto error_end_index = error.find_first_of('\n');
  file << "[ "
       << error.substr(0, error_end_index ? error_end_index : error.length())
       << " ]" << std::endl;
  file << "  ^ lexical error" << std::endl;
}

void write_to_file(std::string const &source_file_name,
  ...
  for (auto const &token : tokens) {
    if (token.type == WHITESPACE)
      continue;
    outfile << "<" << TokenNames.at(token.type) << ", " << token.value << ">"
            << std::endl;
  }
}
